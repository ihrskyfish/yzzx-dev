...
<script>
  ...
  export default {
    name: "MmccCommunicationCircleModal",
    data () {
      return {
        modal: {
          width: 800,
        ...
        url: {
          add: "/communication/circle/add",
          edit: "/communication/circle/edit",
          queryById: "/communication/circle/queryById"
        },
      }
    },
    created () {
      ...
      edit (record) {
        ...
        if(record.id) {
           getAction(this.url.queryById + '?id=' + record.id,{}).then((res)=>{
            if(res.success){
              this.$nextTick(() => {
			    this.form.setFieldsValue(pick(res.result,'title','content'))
              });
            }
          }).finally(() => {
          })
