      ...
      edit (record) {
        ...
        if(record.id) {
           getAction(this.url.queryById + '?id=' + record.id,{}).then((res)=>{
            if(res.success){
              this.$nextTick(() => {
			    this.form.setFieldsValue(pick(res.result,'categoryId','name'))
              });
            }
          }).finally(() => {
          })
          ...
          this.$nextTick(() => {
			  this.form.setFieldsValue(pick(this.model,'categoryId','name'))
			  //时间格式化
          });
        }
