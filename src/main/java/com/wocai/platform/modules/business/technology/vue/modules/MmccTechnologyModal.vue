    ...
    data () {
      return {
        ...
        url: {
          ...
          queryById: "/technology/queryById"
        },
      }
    },
    created () {
      ...
      edit (record) {
        ...
        if(record.id) {
           getAction(this.url.queryById + '?id=' + record.id,{}).then((res)=>{
            if(res.success){
              this.$nextTick(() => {
			    this.form.setFieldsValue(pick(res.result,'name','content','status'))
              });
            }
          }).finally(() => {
          })
          ...
          this.$nextTick(() => {
			  this.form.setFieldsValue(pick(this.model,'name','content','status'))
			  //时间格式化
          });
        }
