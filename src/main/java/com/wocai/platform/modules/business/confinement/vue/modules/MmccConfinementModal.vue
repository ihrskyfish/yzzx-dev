    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          label="活动类型">
          <a-input placeholder="请输入活动类型" v-decorator="['type', validatorRules.type]" />
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="查看次数">
          <a-input placeholder="请输入查看次数" v-decorator="['number', validatorRules.number]" />
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        title:{rules: [{ required: true, message: '请输入活动标题!' }]},
        content:{rules: [{ required: true, message: '请输入内容!' }]},
        number:{rules: [{ required: true, message: '请输入查看次数!' }]},
        },
        url: {
          add: "/confinement/add",
          edit: "/confinement/edit",
          queryById: "/confinement/queryById"
        },
      }
    },
    created () {
      ...
      edit (record) {
        ...
        if(record.id) {
           getAction(this.url.queryById + '?id=' + record.id,{}).then((res)=>{
            if(res.success){
              this.$nextTick(() => {
			    this.form.setFieldsValue(pick(res.result,'type','title','content','number'))
              });
            }
          }).finally(() => {
          })
          ...
          this.$nextTick(() => {
			  this.form.setFieldsValue(pick(this.model,'type','title','content','number'))
			  //时间格式化
          });
        }
