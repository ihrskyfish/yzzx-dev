    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          label="预约接待员姓名">
          <a-input placeholder="请输入预约接待员姓名" v-decorator="['name', validatorRules.name]" />
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
        name:{rules: [{ required: true, message: '请输入预约接待员姓名!' }]},
        },
        url: {
          add: "/servicePerson/add",
          edit: "/servicePerson/edit",
          queryById: "/servicePerson/queryById"
        },
      }
    },
    created () {
      ...
      edit (record) {
        ...
        if(record.id) {
           getAction(this.url.queryById + '?id=' + record.id,{}).then((res)=>{
            if(res.success){
              this.$nextTick(() => {
			    this.form.setFieldsValue(pick(res.result,'name'))
              });
            }
          }).finally(() => {
          })
